...
namespace IamI.Lib.Basic.Time
{
    ...
    public static class TimeHelp
    {
        ...
        ///     分钟的字符后缀
        /// </summary>
        public static readonly List<string> MinuteNames =
            new List<string> {"m", "min", "mins", "minute", "minutes", "分", "分钟"};

        /// <summary>
        ///     秒钟的字符后缀
        /// </summary>
        public static readonly List<string> SecondNames =
            new List<string> {"s", "sec", "secs", "second", "seconds", "秒", "秒钟"};
        ...
        public static TimeSpan ToTimespan(this string timespan_string)
        {
            ...
            if (second_name != null)
                timespan_string = timespan_string.Substring(0, timespan_string.Length - second_name.Length);
            ...
            if (minute_name != null)
            {
                ...
                num = Convert.ToDouble(components[0]) * 60 +
                      (components.Length >= 2 ? (double.TryParse(components[1], out var i) ? i : 0) : 0);
            }
            else
            {
                num = double.TryParse(timespan_string, out var j) ? j : 0;
            }
