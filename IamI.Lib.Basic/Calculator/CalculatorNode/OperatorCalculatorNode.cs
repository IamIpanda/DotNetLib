    ...
    public class OperatorCalculatorNode : CalculatorNode
    {
        ...
        public static double CalculateOperator(double x, double y, char _operator)
        {
            if (_operator == '/' && y == 0 || _operator == '%' && y == 0)
                Logger.Default.Warning("Divided by zero in calculator.");
            else if (_operator == '(' || _operator == ')')
                Logger.Default.Warning("Trying to calculate a operator '(' or ')'.");
            switch (_operator)
            {
                case '+':
                    return x + y;
                ...
                case ',':
                    return y;
                default:
                    Logger.Default.Warning($"Unknown operator in calculator: {_operator}. Would return 0.");
                    return 0;
            }
        }
