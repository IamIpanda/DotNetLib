...
namespace Iami.Lib.Language.Transform
{
    public class zh_CN
    {
        ...
        {
            // 转换成 ASCII 码
            var bytes = Encoding.Default.GetBytes(origin);
            if (bytes.Length != 2)
                return null;
            var value = bytes[0] * 256 + bytes[1];
            if (value < 0xA1A1 || value > 0xFEFE)
                return null;
            value -= 65536;
            // 勘误
        ...
        public IEnumerable<string> Split(string origin)
        {
            return origin.Select(c => c.ToString());
        }
